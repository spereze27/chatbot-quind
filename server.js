...
// URL interna del servicio QuindBot (bot de WhatsApp)
// En Cloud Run se inyecta como variable de entorno BOT_INTERNAL_URL
...
async function bqQuery(sql, params = []) {
    ...
    if (params && params.length > 0) {
        ...
        params.forEach(p => { parametrosFormateados[p.name] = p.value; });
...
async function analizarRiesgoTransferencia({ cedula, monto, dispositivoHash }) {
    ...
    // Saldo insuficiente — rechazar de inmediato
    ...
    // Monto inusualmente alto vs promedio histórico (> 2×)
    ...
    // Más de 3 transferencias completadas en la última hora
...
app.post('/api/auth/registro', async (req, res) => {
    try {
        ...
        res.json({
            ok:       true,
            numeroCuenta,
            mensaje:  'Cuenta creada exitosamente. Saldo inicial: $50,000 COP'
        });
    } catch (err) {
...
app.post('/api/transferencia', async (req, res) => {
    try {
        ...
        const analisis = await analizarRiesgoTransferencia({
            cedula: cedulaOrigen, monto, dispositivoHash
        });
        ...
        if (estadoTx === 'COMPLETADA') {
            ...
            await bqQuery(
                `INSERT INTO ${TBL_MOVIMIENTOS} (cedula, fecha, detalle, movimiento)
                 VALUES (@ced, CURRENT_DATE(), @det, @monto)`,
                [
            ...
            // ── Bloquear cuenta bajo investigación ──
            await bqQuery(
